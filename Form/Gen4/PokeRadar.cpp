...
static Lead getPokeRadarGeneratorLead(const PokeRadarControls &controls)
{
    ...
    if (controls.chainCount->value() == 0 || isPokeRadarChainLead(lead) || isPokeRadarEncounterModifier(lead)
        || lead == Lead::CompoundEyes)
    {
        return lead;
    }
...
QGroupBox *PokeRadar::createRNGInfo(PokeRadarControls &controls, bool searcherTab)
{
    ...
    else
    {
        ...
        connect(controls.lead->menu(), &QMenu::triggered, this,
                [this](QAction *) { updateMinimumGraceSteps(generator, generatorEncounters); });
    }
...
QGroupBox *PokeRadar::createSettings(PokeRadarControls &controls, bool searcherTab)
{
    ...
    layout->addWidget(new QLabel(tr("Cooldown Steps"), settings), levelRow + 1, 0);
...
{
    QModelIndex index = ModelIndexMapping::sourceRow(generator.tableView->currentIndex());
    if (!index.isValid())
    {
        return;
    }
    ...
    for (int row = 0; row < generator.model->rowCount(); row++)
    {
        ...
        {
            QModelIndex target = ModelIndexMapping::fromSource(generator.model->index(row, 0), generator.tableView->model());
...
{
    QModelIndex index = ModelIndexMapping::sourceRow(controls.tableView->currentIndex());
    if (!index.isValid())
    {
        return;
    }
...
{
    QModelIndex index = ModelIndexMapping::sourceRow(searcher.tableView->currentIndex());
    if (!index.isValid())
    {
        return;
    }
