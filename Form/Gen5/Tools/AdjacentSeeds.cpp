...
void AdjacentSeeds::generate()
{
    ...
    if (model->rowCount() > 0)
    {
        ...
        ui->tableView->selectRow(ModelIndexMapping::fromSource(model->index(targetRow, 0), ui->tableView->model()).row());
    }
...
void AdjacentSeeds::updatePreview()
{
    ...
    QModelIndex index = ModelIndexMapping::sourceRow(ui->tableView->selectionModel()->currentIndex());
    if (!index.isValid())
    {
        ui->lineEditPreview->clear();
        return;
