...
WildSearcher4::WildSearcher4(u32 minAdvance, u32 maxAdvance, u32 minDelay, u32 maxDelay, Method method,
    ...
    WildSearcher4(minAdvance, maxAdvance, minDelay, maxDelay, method, leads.empty() ? Lead::None : leads.front(), feebasTile, shiny, unownRadio,
                  happiness, area, profile, filter)
{
    this->leads = leads.empty() ? std::vector<Lead> { Lead::None } : leads;
}

WildSearcher4::WildSearcher4(u32 minAdvance, u32 maxAdvance, u32 minDelay, u32 maxDelay, Method method,
                             const std::vector<Lead> &leads, bool feebasTile, bool shiny, bool unownRadio, u8 happiness,
                             bool searchStepEncounter, u16 stepOptions, const EncounterArea4 &area, const Profile4 &profile,
                             const WildStateFilter &filter) :
    WildSearcher4(minAdvance, maxAdvance, minDelay, maxDelay, method, leads.empty() ? Lead::None : leads.front(), feebasTile, shiny, unownRadio,
                  happiness, searchStepEncounter, stepOptions, area, profile, filter)
{
    this->leads = leads.empty() ? std::vector<Lead> { Lead::None } : leads;
}
...
std::vector<WildSearcherState4> WildSearcher4::search(u8 hp, u8 atk, u8 def, u8 spa, u8 spd, u8 spe, u8 index) const
{
    ...
    {
        states = shiny ? searchPokeRadarShiny(hp, atk, def, spa, spd, spe, index) : searchPokeRadar(hp, atk, def, spa, spd, spe, index);
    }
...
std::vector<WildSearcherState4> WildSearcher4::searchPokeRadarShiny(u8 hp, u8 atk, u8 def, u8 spa, u8 spd, u8 spe, u8 index) const
{
    ...
    for (int i = 0; i < seeds.count; i++)
    {
        ...
        bool synchronize = isSynchronizeLead(lead);
        if (synchronize || cuteCharm)
        {
            u8 huntNature;
