...
#include <PokeFinderPlus/Branding.hpp>
...
void logPokeFinderPlusStartup()
{
    ...
    std::wstring applicationDirectory(modulePath);
    const size_t separator = applicationDirectory.find_last_of(L"\\/");
    applicationDirectory.resize(separator);
    QSettings diagnostics(QString::fromStdWString(applicationDirectory) + "/plugins.ini", QSettings::IniFormat);
    if (!diagnostics.value("diagnostics/logging", false).toBool())
    {
        return;
    }
    const std::wstring logPath = applicationDirectory + L"\\PokeFinderPlus.log";
    HANDLE log = CreateFileW(logPath.c_str(), FILE_APPEND_DATA, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_ALWAYS,
                             FILE_ATTRIBUTE_NORMAL, nullptr);
    if (log != INVALID_HANDLE_VALUE)
    {
        const std::string lines = "applicationDirPath=" + QString::fromWCharArray(applicationDirectory.c_str()).toStdString() +
                      "\ncurrentWorkingDirectory=" + QString::fromWCharArray(currentPath).toStdString() + "\n";
        DWORD written = 0;
        WriteFile(log, lines.data(), static_cast<DWORD>(lines.size()), &written, nullptr);
        CloseHandle(log);
    }
#endif
}
