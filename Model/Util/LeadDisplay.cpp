...
namespace LeadDisplay
{
    QString getLeadName(u64 leadMask, bool showSynchronizeNature)
    {
        ...
        if (has(leadMask, Lead::RockSmashMagnetPull))
        {
            leads.append(trLead("Magnet Pull"));
        }
        if (has(leadMask, Lead::RockSmashSuctionCups))
        {
            leads.append(trLead("Suction Cups"));
        }
        if (has(leadMask, Lead::RockSmashSuperLuck))
        {
            leads.append(trLead("Super Luck"));
        }
        if (has(leadMask, Lead::KeenEye))
        {
            leads.append(trLead("Keen Eye"));
        }
        if (has(leadMask, Lead::Intimidate))
        {
            leads.append(trLead("Intimidate"));
        }
        if (has(leadMask, Lead::SereneGrace))
        {
            leads.append(trLead("Serene Grace"));
        }
