...
int main(int argc,char **argv)
{
    ...
    app.setOrganizationName("WildOptimizationVerification"); app.setApplicationName("WildOptimizationVerification");
    ...
    ProfileLoader5::setProfiles({Profile5("Wild milestone 2",Game::Black,12345,54321,"","",0x001122334455ULL,
    ...
    try
    {
        ...
        std::array<bool,16> hp{}; hp[13]=true; child<CheckList>(*filter,"checkListHiddenPower")->setChecks(hp);
        check(filter->getHiddenPowers()==hp,"Hidden Power Ice selected");
        ...
        check(baseline.size()==594000 && optimized.size()==baseline.size(),"practical setup returns 594000 identical-count results");
        ...
        if(argc>1) wild.grab().save(QString::fromLocal8Bit(argv[1])+"/normal-wild-test.png");
