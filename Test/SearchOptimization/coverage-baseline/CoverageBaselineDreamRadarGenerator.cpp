...
#include "CoverageBaselineDreamRadarGenerator.hpp"
...
CoverageBaselineDreamRadarGenerator::CoverageBaselineDreamRadarGenerator(u32 initialAdvances, u32 maxAdvances, u8 badgeCount,
...
std::vector<DreamRadarState> CoverageBaselineDreamRadarGenerator::generate(u64 seed) const
{
    ...
    for (u32 cnt = 0; cnt <= maxAdvances; cnt++, rngList.advanceStates(2), rng.next())
    {
        ...
        u32 pid = Utilities5::createPID(tsv, 2, radarTemplate.getGender(), Shiny::Never, false, info->getGender(), go);
