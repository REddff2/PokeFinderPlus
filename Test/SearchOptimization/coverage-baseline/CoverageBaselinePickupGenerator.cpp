...
#include "CoverageBaselinePickupGenerator.hpp"
...
CoverageBaselinePickupGenerator::CoverageBaselinePickupGenerator(u32 initialAdvances, u32 maxAdvances, const std::array<Slot, 6> &pickupSlots,
                                 const std::vector<Request> &requests, const EncounterArea5 &area, const Profile5 &profile,
                                 bool includeInvalid) :
    CoverageBaselinePickupGenerator(initialAdvances, maxAdvances, pickupSlots, area, profile, createDefaultFilter(), includeInvalid)
{
    this->requests = requests;
}

CoverageBaselinePickupGenerator::CoverageBaselinePickupGenerator(u32 initialAdvances, u32 maxAdvances, const std::array<Slot, 6> &pickupSlots, const EncounterArea5 &area,
...
std::vector<PickupState> CoverageBaselinePickupGenerator::generate(u64 seed, u32 ivAdvances) const
{
...
std::vector<u16> CoverageBaselinePickupGenerator::getLevelItems(u8 level)
{
    if (level == 0)
    {
        return {};
...
bool CoverageBaselinePickupGenerator::compare(const std::array<u16, 6> &items) const
{
    if (!requests.empty())
    {
...
u16 CoverageBaselinePickupGenerator::getPickupItem(u8 level, u8 itemRand) const
{
    u8 range = levelRange(level);
